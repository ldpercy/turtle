...
Not going to labour over this one too much, have done a basic conversion, and will polish as things progress and I learn more.

* Added warning if page loaded with `file://` protocol
* `index.html` now only loads the `turtleApp.js` module, everything else loaded from there (and dependencies)
* Under http caching is now a problem, switch it off in browser dev tools
* I've converted Maths from a class (of statics) back to a library; all other classes as before for now
* A bunch of readme and wiki changes
* A few small UI tweaks such as adding in the turtle's name
* Added a very primitive draw of 'Turtle' for the initial load. I'll make it better I promise.
