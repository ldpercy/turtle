...
```
2025-11-20		Start 🖮🎛
2025-12-10		v0.10 done
2025-12-11		v0.10.1 followups done
```
...
* Tried out some styles for ninja turtles -> [multi turtle](<ꙮ🐢 - multi-turtle.md>)
...
I remembered how to stop FF's 'find-as-you-type' that I use in my regular profiles - it was messing with the keyboard controls:

	https://developer.mozilla.org/en-US/docs/Web/API/Event/preventDefault

Will add this to the keyboard listener.
Also adding `z` & `Z` as options for zoom in/out - plus and minus were a bit awk on my laptop.

### Prevent default & key mapping.

I've just found out that it's not a good idea to add 'preventDefault' to all keyboard events.
It's rather bad in fact.
So I only need to add it to keys that I want mapped, which means I think I need to rewrite how I've done that.


### Popover
The next thing I want to add here is an info popover of some sort - I'd just begun playing with these a couple of weeks back.

Have quickly jammed something together, kind of okay.
Really need to clean up the html css, it's getting a bit out of control.

Added.
Needs work - couldn't get the backdrop blur working properly, but good enough for now.

Wrapup 2
========

* Convert key switch statement to key function map;
* Add preventDefault for mapped keys
* Added `z`/`Z` key options for zoom in/out
* Added new info popover (`?`) - combines the old command & project info summaries with the new key shortcut list
* The backdrop isn't quite right yet...
